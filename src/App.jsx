...
const STORAGE_KEYS = {
  ...
  hostNightJournalPrefix: "lg_host_night_journal_",
}
...
function App() {
  ...
  const [hostNightJournal, setHostNightJournal] = useState("")
  ...
  function getHostNightJournalStorageKey(gameId) {
    return `${STORAGE_KEYS.hostNightJournalPrefix}${gameId}`
  }
  ...
  useEffect(() => {
    ...
    const journalKey = getHostNightJournalStorageKey(currentGame.id)
    setHostNightJournal(localStorage.getItem(journalKey) || "")
  ...
  function HostGameScreen() {
    ...
    if (activeHostTab === "log") {
      return (
        <div style={styles.screen} className="screen-enter">
          {HeaderBar({
            title: "Journal de nuit",
            onBack: () => setActiveHostTab("game"),
            right: <Moon size={20} />,
          })}

          <div style={{ paddingTop: 18, display: "grid", gap: 16, paddingBottom: 100 }}>
            <div style={styles.glassCard} className="floating-card">
              <div style={{ ...styles.sectionTitle, marginBottom: 10 }}>Pendant la nuit</div>
              <p style={{ ...styles.subtitle, margin: 0, color: "#b8c3e0" }}>
                Note tout ce qui se passe (loups, pouvoirs, décisions). Le texte est enregistré sur cet appareil.
              </p>
              <div
                style={{
                  marginTop: 14,
              ...
              >
                <div style={styles.badge}>Nuit {currentGame.night_number || 1}</div>
                <div style={{ ...styles.badge, letterSpacing: "0.06em" }}>
                  {getPhaseLabel(currentGame.phase)}
                </div>
              </div>
            </div>
            ...
            <textarea
              value={hostNightJournal}
              onChange={(e) => {
                const next = e.target.value
                setHostNightJournal(next)
                localStorage.setItem(getHostNightJournalStorageKey(currentGame.id), next)
              }}
              placeholder="Ex. Nuit 1 — Les loups visent Marie. La voyante sonde Paul (villageois). La sorcière garde sa potion de vie…"
              style={{
                width: "100%",
                minHeight: 360,
                resize: "vertical",
          ...
          {BottomNav({
            items: [
              ...
              { key: "log", label: "Journal", icon: ScrollText, active: activeHostTab === "log" },
    ...
    return (
      <div style={styles.screen} className="screen-enter">
        ...
        <div style={{ paddingTop: 24, display: "grid", gap: 22 }}>
          ...
          {!ended ? (
            <button onClick={goToNextPhase} style={{ ...styles.primaryBtn, marginTop: 0 }}>
